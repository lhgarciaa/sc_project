...
while (( $(echo "$gamma <= $max_gamma" | bc -l) )); do
    ...
    qsub -N $job_name -cwd -t 1-$num_runs -o $log_name_base.out -e $log_name_base.err -q compute.q src/qsub_und_run_louvain_row_col_ctx_mat.sh $row_column_input_csv $gamma $num_runs $mod_dir $venv_dir
