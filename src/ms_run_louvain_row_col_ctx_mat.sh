...
while (( $(echo "$gamma <= $max_gamma" | bc -l) )); do
    ...
    qsub -cwd -pe smp 8-16 -binding linear:8-16 -o $log_name_base.out -e $log_name_base.err -q compute.q src/qsub_run_louvain_row_col_ctx_mat.sh $row_column_input_csv $gamma $num_runs $venv_dir
