...
def main():
    ...
    for idx in xrange(ant_row_roi_name_npa.size):
        ...
        a_row = map(lambda x: normalize_calc(row_max, row_min, x), ant_ctx_mat_npa[idx])
    ...
    for idx in xrange(ret_col_roi_name_npa.size):
        ...
        a_col = map(lambda x: normalize_calc(col_max, col_min, x), ret_ctx_mat_npa[:, idx])
    ...
    with open(output_ctx_mat_csv, 'wb') as csvfile:
        ...
        header_row = [''] + ant_col_roi_name_npa.tolist() + ret_col_roi_name_npa.tolist()
        csvwriter.writerow(header_row)

        for idx in xrange(ant_row_roi_name_npa.size):
            ant_row = [ant_row_roi_name_npa[idx]] + ant_norm_ctx_mat[idx] + ([''] * ret_col_roi_name_npa.size)

            csvwriter.writerow(ant_row)

        for idx in xrange(ret_row_roi_name_npa.size):
            ret_row = [ret_row_roi_name_npa[idx]] + ([''] * ant_col_roi_name_npa.size) + list(ret_ctx_mat_npa[idx])

            csvwriter.writerow(ret_row)

    print("Wrote combined connection matrix csv to {}".format(output_ctx_mat_csv))
