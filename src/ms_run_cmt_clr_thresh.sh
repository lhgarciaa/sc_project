...
if [ ! -n "$5" ]
then
    echo "Usage: `basename $0` <char-cmt-str-csv> <case-dir> <grid-cell-size> <channel> <venv-dir>"
    exit 1
fi
...
while (( $(echo "$lvl <= $max_lvl" | bc -l) )); do
    ...
    qsub -N $job_name -cwd -o $log_name_base.out -e $log_name_base.err -q compute.q src/qsub_run_cmt_clr_thresh.sh $char_cmt_str_csv $case_dir $grid_cell_size $channel $lvl $venv_dir
    lvl=$(echo "$lvl + 1" | bc);
done;
