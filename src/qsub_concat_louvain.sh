...
if [ ! -n "$3" ]; then
    echo "Usage: `basename $0` <row-column-input-csv> <gamma> <num-runs>"
    exit 1
fi
...
cat_file=$root_dir/mod-${print_gamma}_runs-${num_runs}_$name_base
...
if [ $num_files -eq $num_runs ]; then
	...
	cat $mod_dir/out-${i}_mod-${print_gamma}_runs-${num_runs}_$name_base >> $cat_file;
    done;
    if [ -e $cat_file ]; then
